...
| Item (VERIFY #) | What TRIPOD-LLM requires | How AIVMT addresses it | STATUS |
|---|---|---|---|
| **Sample size / power** (VERIFY ~Item 8) | Justify sample size for the evaluation. | Pre-specified and reasoned on the 95% CI half-width of ICC(2,1), k=3: primary **n = 150 encounters × 3 raters** (half-width ≈0.07 @ICC 0.70), floor **n = 100**, staged pilot ~25, contingency n≈40 (`HYPOTHESIS.md` "Sample size / power" l.39–52). These cite `prereg §11 l.107–118` and `PROJECT_PLAN l.40/67`; **both source files now exist on disk** (authored 2026-06-13) and the §/line anchors resolve (see §8 below). The power *logic* is also locked in `HYPOTHESIS.md`. | **Partial** (power justification pre-specified and now sourced to prereg §11; final n pending real rater-hours) |
...
| Item (VERIFY #) | What TRIPOD-LLM requires | How AIVMT addresses it | STATUS |
|---|---|---|---|
| **Study registration / protocol** (VERIFY ~Item 14) | State registration and where the protocol/analysis plan can be found. | A preregistration/analysis plan is **referenced throughout** `HYPOTHESIS.md` as `plan/preregistration-v0.1.md` (with cited §/line numbers, e.g. §4 l.22–24, §8 l.67–75, §10 l.90–96, §11 l.107–118) and `plan/PROJECT_PLAN.md` (l.40, l.67). **Both files now exist on disk** (authored 2026-06-13; `find` resolves them and the cited §/line anchors resolve), so the analysis plan is *pre-specified AND reproducible* from the prereg artifact, not only `HYPOTHESIS.md`. Residual: **external registration (e.g., OSF) is TODO_COLLAB**, and the load-bearing line anchors must be re-verified whenever prereg sections move. | **TODO_COLLAB** (external OSF registration pending; prereg/plan files now on disk + anchors resolve) |
...
1. **Add external study registration (e.g., OSF); keep the load-bearing prereg line anchors in sync.**
   The cited `plan/preregistration-v0.1.md` and `plan/PROJECT_PLAN.md` now **exist on disk** (authored
   2026-06-13) and their §/line anchors resolve, so the analysis plan is reproducible from the prereg
   artifact itself. Residual: obtain an external registration (e.g., OSF) for the protocol, and
   re-verify the **load-bearing** §/line anchors in `HYPOTHESIS.md` and the prereg whenever sections
   move. *(§8 registration row.)*
